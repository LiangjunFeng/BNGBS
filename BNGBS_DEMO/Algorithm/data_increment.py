...
class broadnet_enhmap:
    ...
    def adding_data(self, data, xdata_list, xdata, xlabel):  
...
class GradientBoostingNet:
    ...
    def fit(self,traindata,trainlabel):
        ...
        for i in range(self._n_estimators):            
            ...
            self.trainlabel_list.append(label)
    ...
    def adding_data(self, data, label, xdata, xlabel):
        
        starttime = datetime.datetime.now()
        label = self.onehot.transform(np.mat(label).T)
        data = self.scaler.transform(data)
        xlabel = self.onehot.transform(np.mat(xlabel).T)
        xdata = self.scaler.transform(xdata)
        
        for i in range(self._n_estimators):
            net = self.net_list[i]
            net.adding_data(data, self.xdata_list, xdata,xlabel)
